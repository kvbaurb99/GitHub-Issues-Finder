...
export default function Issue({title, state, date, comments, link, description}) {
  ...
  return (
    <div onMouseLeave={() => setIsHover(false)} onMouseOver={() => setIsHover(true)} className={`text-white border border-gray-600 mt-4 p-4 cursor-pointer ${isHover ? 'bg-gray-600/30' : null} h-full rounded relative`}>
        <a href={link} rel='noreferrer' target='_blank'><p className={`font-bold cursor-pointer text-xl tracking-wider max-w-[90%] ${isHover ? 'text-orange-500' : null}`}>{title}</p></a>
        ...
        <div className='flex items-center gap-2 text-sm text-gray-400 mt-4'>
            <p className={`${isHover ? 'text-orange-500 font-bold' : null} tracking-wide`}>Technology: <span>{technology}</span></p>
            <p className={`${isHover ? 'text-orange-500 font-bold' : null} tracking-wide`}>Created at: <span>{formattedDate}</span></p>
            <p className={`${isHover ? 'text-orange-500 font-bold' : null} tracking-wide`}>Comments: <span>{comments}</span></p>
